...
{  #molecular.pri <- mutation.pri[,keep.index]
  ...
  write.table(t(rbind(rownames(clinical.common),ifelse(clinical.common$Z==1, "normal","overweight"))), file=paste(cancer,"_",data.type,"_samples.txt", sep=""),quote = F,row.names = F,sep="\t")
  
  print("----------------")
  if("age_at_index" %in% colnames(clinical.common)){
    age.cutoff <- 65
    print(paste("age <", age.cutoff,":", length(which(clinical.common$age_at_index < age.cutoff))))
    print(paste("age >=", age.cutoff,":", length(which(clinical.common$age_at_index >= age.cutoff))))
    print("----------------")
  ...
  ##loading PSM
  source("./code/GIPW_function_omega.R")
