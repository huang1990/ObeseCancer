setwd("./data/processedData/mutationSignature/")
...
for (d in disease) {
  for (i in group) {
    ...
    data <- read.csv(paste("./data/processedData/clinical/",d,"_clinicalData.csv",sep = ""),header=T,stringsAsFactors=F)
    head(data)
    ...
    # MutationSignature
    mutation <- read.table(paste(d,".signature.tran.txt", sep = ""), sep = "\t", header = T)
    ##SBS: X
    mutationFeature <- mutation[substr(mutation$Sample,14,15) %in% c("01"),] %>% dplyr::mutate(sampleID=substr(Sample,1,12))
    head(mutationFeature)
    Feature.pri <- mutationFeature[!duplicated(mutationFeature$Sample),]
    head(Feature.pri)
    #Feature.pri <- mutationFeature[5:ncol(mutationFeature)]
    rownames(Feature.pri) <- Feature.pri$sampleID
    Feature.pri$Sample<- NULL
    Feature.pri$sampleID <- NULL
    head(Feature.pri)
    ...
    write.summary(sum.Immune, d, analysis,"MutationSig")
    write.result(Feature.result, d, analysis,"MutationSig")
    save(Feature.result, file=paste(d,"_", analysis,"_MutationSig_result.RData",sep=""))
  }
}
