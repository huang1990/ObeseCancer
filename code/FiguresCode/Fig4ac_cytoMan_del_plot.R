...
cytoband_new <- cytoband[,c("cy","cyto","chr", "result.fdr","result.pvalue", "cytoType", "bias")]
...
don = don %>% mutate(color = if_else(bias == "Obesity_biased", 'hotpink3','wheat4'))
...
##deletion
don<- don %>%
  mutate(fdr = case_when(cytoType == "del" ~ P,
                         cytoType == "am" ~ 1))
-log10(don$fdr)
don$fdr[is.na(don$fdr)] = 1

pdf(file = "./Figures/Fig4a_ESCA_del.pdf", 
    width=3,
    height=8)

ggplot() +
  geom_rect(data = data1, mapping=aes(xmin = start, xmax = end, ymin = 0, ymax = 2.2),fill="gray90", color = "gray90") +
  ...
  coord_flip() + 
  labs(title = "ESCA Deletions", y = "-log10 (FDR)", x = "") +
...
pdf(file = "./Figures/Fig4b_UCEC_cytoband_del.pdf",
    width=3,
    height=8)
...
ggplot() +
  ...
  geom_hline(yintercept=1, linetype="dashed", color = "green") +
  coord_flip() + 
  labs(title = "UCEC Deletions", y = "-log10 (FDR)", x = "") +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(),
        title = element_text(size=14, color="black"),
        # axis.line = element_line(colour = "black"),
